    ...
    items: content.actions.map(action => ({
      ...
      component: (
        <GridCard
          ...
          actions={
            <BaseButton
              variant={action.buttonVariant === 'contained' ? 'primary' : 'secondary'}
              customSx={{
                backgroundImage: action.buttonVariant === 'contained' ? 'linear-gradient(45deg, #2196F3 30%, #9C27B0 90%)' : undefined,
              }}
            >
              {action.buttonText}
            </BaseButton>
